...
type DetailCompletedModalProps = {
  isOpen: boolean;
  onClose: () => void;
  vehicle: {
    id: string;
...
export function DetailCompletedModal({ isOpen, onClose, vehicle, onSuccess }: DetailCompletedModalProps) {
  ...
  const handleSubmit = async () => {
    if (!handedTo.trim()) {
      alert("Please enter who is taking the vehicle next.");
      return;
    }
    ...
    try {
      const { error } = await supabase.from('recon_jobs').update({
        current_stage_code: 'detail-inspection', // Move to next stage
        final_decision_notes: `DETAIL COMPLETED. Handed to: ${handedTo}`,
      ...
      <div className="bg-white/95 backdrop-blur-sm border border-[#E0E0E0] rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-300">
        ...
        <div className="p-10 pb-6 flex items-center justify-between">
          <div className="space-y-2">
            <h2 className="text-4xl font-black text-[#333333] tracking-tighter uppercase leading-none">Detail Completed</h2>
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest opacity-60">
              Finalizing aesthetic work for: <span className="text-[#00142E]">{vehicle.label}</span>
            </p>
          </div>
        ...
        <div className="p-10 pt-0 space-y-8">
          <div className="space-y-3">
            <label className="text-[9px] font-black text-gray-500 uppercase tracking-[0.3em] ml-1">Hand over to [Next Responsible Person]</label>
            <div className="relative group">
              <input 
                ...
                placeholder="WHO IS TAKING THE VEHICLE NEXT?"
          ...
          >
            {isSubmitting ? "PROCESSING..." : "Confirm Detail Completion"}
          </button>
        </div>
      </div>
    </div>
